...
class Experiment:
    def __init__(self, generator, label_col,  model, n_targets, n_shadows, hyperparams, n_queries=1024, n_classes=2, range=None):
        ...
        Args:
            ...
            range (tuple): the range of values accepted for regression tasks (needed for regression, ignored for classification)
                         it is possible to pass an iterable of multiple ranges in order to perform multi-variable property inference regression,
                         in which case the values of the variables are passed to the Generator as a list
        """

        assert isinstance(generator, Generator), 'The given generator is not an instance of Generator, but {}'.format(
            type(generator).__name__)
        self.generator = generator

        assert isinstance(label_col, str), 'label_col should be a string, but is {}'.format(
            type(label_col).__name__)
        self.label_col = label_col

        assert issubclass(
            model, Model), 'The given model is not a subclass of Model'
        self.model = model

        assert isinstance(n_targets, int), 'The given n_targets is not an integer, but is {}'.format(
            type(n_targets).__name__)
        self.n_targets = n_targets

        assert isinstance(n_shadows, int), 'The given n_shadows is not an integer, but is {}'.format(
            type(n_shadows).__name__)
        ...
        assert isinstance(n_queries, int), 'The given n_queries is not an integer, but is {}'.format(
            type(n_queries).__name__)
        self.n_queries = n_queries

        assert isinstance(n_classes, int), 'The given n_classes is not an integer, but is {}'.format(
            type(n_classes).__name__)
        if n_classes == 1:
        ...
        reg = self.model(self.label_col, self.hyperparams).fit(
    ...
    def __optimise_hyperparams(self):
        ...
        for params in optims:
            ...
            for i in range(len(train)):
                models = [self.model(self.label_col, hyperparams).fit(
                    train[i]) for _ in range(10)]

                if not reg_checked and \
                        (len(models[0].predict_proba(test[0]).shape) < 2
                         or models[0].predict_proba(test[0]).shape[1] == 1):
                    is_reg = True

                reg_checked = True

                if is_reg:
                    res -= np.mean([mean_squared_error(test[i][self.label_col],
                                   m.predict(train[i])) for m in models])
                else:
                    res += np.mean([accuracy_score(test[i][self.label_col],
                                   m.predict(train[i])) for m in models])
        ...
        logger.debug(
            'Best hyperparameters defined as: {}'.format(self.hyperparams))
        if is_reg:
    ...
    def run_targets(self):
        ...
        if self.n_classes > 1:
            ...
            if hasattr(self.range[0], '__getitem__'):
                ...
                self.labels = np.random.uniform(
                    bounds[:, 0], bounds[:, 1], (self.n_targets, len(self.range)))
            else:
                self.labels = np.arange(
                    self.range[0], self.range[1], (self.range[1] - self.range[0])/self.n_targets)
        else:
            raise AttributeError(
                "Invalid n_classes provided: {}".format(self.n_classes))
    ...
    def run_shadows(self, model=None, hyperparams=None):
        ...
        if self.n_classes > 1:
            ...
            if hasattr(self.range[0], '__getitem__'):
                bounds = np.array(self.range)
                self.shadow_labels = np.random.uniform(
                    bounds[:, 0], bounds[:, 1], (self.n_shadows, len(self.range)))
            else:
                self.shadow_labels = np.arange(
                    self.range[0], self.range[1], (self.range[1] - self.range[0])/self.n_shadows)
        else:
            raise AttributeError(
                "Invalid n_classes provided: {}".format(self.n_classes))
    ...
    def run_loss_test(self):
        ...
        accuracy = [[accuracy_score(X[self.label_col], t.predict(X))
                     for X in X_test] for t in self.targets]
    ...
    def run_threshold_test(self, n_outputs=1):
        ...
        accuracy = np.array([[accuracy_score(X[self.label_col], s.predict(X))
                            for X in X_test] for s in self.shadow_models])
        k = np.argmax(np.abs(np.sum(accuracy[shadow_labels, :], axis=0)
                             - np.sum(accuracy[~shadow_labels, :], axis=0)))
        higher_acc = np.argmax(
            [np.sum(accuracy[~shadow_labels, k]), np.sum(accuracy[shadow_labels, k])])

        thr = 0.0
        best_acc = 0.0
        for z in np.arange(0, 1, 1e-2):
            thr_labels = [higher_acc if acc
                          > z else not higher_acc for acc in accuracy[:, k]]
            acc = accuracy_score(shadow_labels, thr_labels)
            if acc > best_acc:
                thr = z
                best_acc = acc

        accuracy = np.array([accuracy_score(
            X_test[k][self.label_col], t.predict(X_test[k])) for t in self.targets])
        y_pred = [higher_acc if acc
                  > thr else not higher_acc for acc in accuracy]
    ...
    def run_whitebox_deepsets(self, hyperparams, n_outputs=1):
        ...
        latent_dim = hyperparams['latent_dim'] if 'latent_dim' in hyperparams.keys(
        ) else 5
        epochs = hyperparams['epochs'] if 'epochs' in hyperparams.keys(
        ) else 20
        lr = hyperparams['learning_rate'] if 'learning_rate' in hyperparams.keys(
        ) else 1e-4
        wd = hyperparams['weight_decay'] if 'weight_decay' in hyperparams.keys(
        ) else 1e-4
        out_dim = 1 if self.n_classes > 1 or not hasattr(
            self.range[0], '__getitem__') else len(self.range)
    ...
    def __get_queries(self):
        if self.n_classes > 1:
            ...
            labels = np.concatenate([[i]*self.generator.n_samples for i in range(self.n_classes)])

            while len(queries) <= self.n_queries:
                queries = pd.concat(queries + [self.generator.sample(i, adv=True) for i in range(self.n_classes)])
                labels = np.concatenate(labels + [[i]*self.generator.n_samples for i in range(self.n_classes)])

        elif self.n_classes == 1:
            if hasattr(self.range[0], '__getitem__'):
                bounds = np.array(self.range)
                labels = np.random.uniform(
                    bounds[:, 0], bounds[:, 1], (10*len(self.range), len(self.range)))
                sample_len = len(self.generator.sample(
                    [0]*len(self.range), adv=True))
            else:
                labels = np.arange(
                    self.range[0], self.range[1], (self.range[1] - self.range[0])/10)
            ...
            while len(queries) <= self.n_queries:
                queries = pd.concat(queries + [self.generator.sample(l, adv=True) for l in labels])
                labels = np.concatenate(labels + [[l]*sample_len for l in labels])
        else:
            raise AttributeError(
                "Invalid n_classes provided: {}".format(self.n_classes))
    ...
    def run_blackbox(self, n_outputs=1):
        ...
        queries, labels = self.__get_queries()
        ...
        train = pd.DataFrame(data=[s.predict(queries).flatten()
                             for s in self.shadow_models])
        test = pd.DataFrame(data=[s.predict(queries).flatten()
